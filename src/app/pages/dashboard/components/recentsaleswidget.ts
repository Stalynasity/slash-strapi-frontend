...
import { WidgetService, Issue } from '../../../shared/services/widget.Service';
@Component({
    ...
    template: `
        <div class="card">
                ...
                <table class="min-w-full table-auto text-sm text-left border-collapse">
                    ...
                    <tbody>
                        <tr *ngFor="let issue of issues" class="border-b">
                            ...
                            <td class="px-4 py-2">
                                <span class="bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs">{{ issue.type }}</span>
                            </td>
                            <td class="px-4 py-2">
                                <span class="bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-xs">{{ issue.priority }}</span>
                            </td>
                            <td class="px-4 py-2">
                                <span class="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs">{{ issue.status }}</span>
                            </td>
                            <td class="px-4 py-2">{{ issue.owner }}</td>
                            <td class="px-4 py-2">{{ issue.createdAt }}</td>
                            <td class="px-4 py-2">
                                <button pButton pRipple icon="pi pi-qrcode" class="p-button-sm p-button-text" (click)="open(issue)"></button>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <p-dialog header="Detalles del Problema" [(visible)]="display" [breakpoints]="{ '900px': '75vw' }" [style]="{ width: '60vw' }" [modal]="true">
                <div class="grid grid-cols-12 gap-4">
                    ...
                    <div class="col-span-12 md:col-span-8">
                        ...
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <span class="text-blue-600 font-semibold text-sm">{{ selectedIssue?.id }}</span>
                        </div>
                        <div class="flex items-center gap-2 mb-4">
                            <span class="text-gray-500 text-sm">Responsable:</span>
                            <span class="text-blue-600 font-semibold text-sm">{{ selectedIssue?.owner }}</span>
                        </div>
                        <div class="flex items-center gap-2 mb-4">
                            <span class="text-gray-500 text-sm">Fecha:</span>
                            <span class="text-gray-600 text-sm">{{ selectedIssue?.createdAt }}</span>
                        </div>
                        ...
                        <div class="flex gap-2">
                            <img src="https://primefaces.org/cdn/primeng/images/demo/product/bug.png" alt="Bug evidencia" class="w-24 h-24 object-cover rounded shadow-md" />
...
export class RecentSalesWidget {
    ...
    handleShare(type: 'email' | 'whatsapp' | 'telegram') {
      ...
      const subject = encodeURIComponent(`Información del incidente ${this.selectedIssue.id}: ${this.selectedIssue.title}`);
      const body = encodeURIComponent(`Descripción: ${this.selectedIssue.descripcion}\nVer detalle del incidente.`);
      const urlQr = window.location.origin + `/bug/${this.selectedIssue.id}`; // Ajusta si tu ruta real es distinta

      if (type === 'email') {
          window.open(`mailto:?subject=${subject}&body=${body}%0A${urlQr}`, '_blank');
      }
      if (type === 'whatsapp') {
          window.open(`https://wa.me/?text=${subject}%0A${body}%0A${urlQr}`, '_blank');
      }
      if (type === 'telegram') {
          window.open(`https://t.me/share/url?url=${urlQr}&text=${subject}%0A${body}`, '_blank');
      }
  }
}
